...
def test_a_sink_is_detected_and_removed():
    ...
    mass, eff_nosink, _, _, tail_mass = sink_and_jensen(q, k, None, sc, sink=1, tail=8)
    assert mass > 0.8
...
def test_without_a_sink_the_count_barely_moves():
    q, k, sc = toy()
    mass, eff_nosink, _, _, _ = sink_and_jensen(q, k, None, sc, sink=4)
    t = k.shape[-2]
...
def test_the_sink_is_counted_among_allowed_positions_only():
    ...
    mass, eff, _, _, _ = sink_and_jensen(q, k, allowed, sc, sink=1)
    assert 0.0 < mass < 1.0
    assert eff >= 1.0
...
def test_jensen_term_is_quadratic_in_the_query_norm():
    ...
    q, k, sc = toy()
    _, _, j1, _, _ = sink_and_jensen(q, k, None, sc)
    _, _, j2, _, _ = sink_and_jensen(2 * q, k, None, sc)
    assert j2 == pytest.approx(4 * j1, rel=1e-6)


def test_jensen_term_matches_the_closed_form():
    q, k, sc = toy()
    _, _, j, _, _ = sink_and_jensen(q, k, None, sc)
    d = k.shape[-1]
